...
REM Depricate direct calls to signtool for non OEM / WPB scenarios to fix issue with package generation
REM which removes any "unapproved" envrionmental variables
...
if "%CERTIFICATEFLAG%"=="app" set CERTIFICATE=/s my /i "Windows OEM Intermediate 2017 (TEST ONLY)" /n "Windows Phone OEM App Test Cert 2013 (TEST ONLY)" /fd SHA256
REM
REM Legacy/old "ext" is translated as "Windows OEM Test Cert 2017 (TEST ONLY)"
REM
if "%CERTIFICATEFLAG%"=="ext" set CERTIFICATE=/s my /i "Windows OEM Intermediate 2017 (TEST ONLY)" /n "Windows OEM Test Cert 2017 (TEST ONLY)" /fd SHA256

if "%CERTIFICATEFLAG%"=="pp" set CERTIFICATE=/s my /i "Windows OEM Intermediate 2017 (TEST ONLY)" /n "Windows Phone OEM PP Test Cert 2013 (TEST ONLY)" /fd SHA256
if "%CERTIFICATEFLAG%"=="ppl" set CERTIFICATE=/s my /i "Windows OEM Intermediate 2017 (TEST ONLY)" /n "Windows Phone OEM PPL Test Cert 2013 (TEST ONLY)" /fd SHA256
...
if "%CERTIFICATEFLAG%"=="pkg" set CERTIFICATE=/s my /i "Windows OEM Intermediate 2017 (TEST ONLY)" /n "Windows OEM Test Cert 2017 (TEST ONLY)" /fd SHA256
goto :SIGNLOOP

:UseWPBCerts
REM IMG2PKG uses WPB certificates as part of resign

set CERTIFICATE=/s my /i "Windows OEM Intermediate 2017 (TEST ONLY)" /n "Windows OEM Test Cert 2017 (TEST ONLY)" /fd SHA256
REM If running in the windows build lab do not enable timestamps
if defined RAZZLETOOLPATH (
    set SIGN_WITH_TIMESTAMP=0
) else (
...
if "%CERTIFICATEFLAG%"=="pkg" set SCENARIO_NAME=MicrosoftCorporationDualSigned
...
echo %RazzleToolPath%\signer.cmd authenticode %SIGNER_SIG_TYPE% -s %SCENARIO_NAME% %FILELIST%
call %RazzleToolPath%\signer.cmd authenticode %SIGNER_SIG_TYPE% -s %SCENARIO_NAME% %FILELIST%
